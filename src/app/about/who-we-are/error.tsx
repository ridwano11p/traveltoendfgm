...
export default function WhoWeAreError({ error, reset }: ErrorProps) {
  const router = useRouter();
  const { theme } = useTheme();
  const isDark = theme === "dark";

  useEffect(() => {
    console.error("Who We Are page error:", error);
  }, [error]);
      ...
      <div className="max-w-xl mx-auto px-4">
        <div className="text-center">
          ...
          <p className={`mb-8 ${isDark ? "text-gray-300" : "text-gray-600"}`}>
            We encountered an error while trying to load our team members. Please try again or return to the homepage.
          </p>
